...
export function startGame(mount: HTMLElement): () => void {
  ...
  heldGroup.add(new THREE.Mesh(
    ...
    new THREE.MeshPhongMaterial({ color: 0xc8a94a, flatShading: true, emissive: new THREE.Color(0xffaa00), emissiveIntensity: 0.5 })
  ));
  heldGroup.position.set(0.3, 0.35, 0);
  heldGroup.add(new THREE.PointLight(0xffaa44, 0.7, 4));
  ...
  function makeLantern(dir: THREE.Vector3): Lantern {
    ...
    const cap = new THREE.Mesh(
      new THREE.ConeGeometry(0.2, 0.18, 4),
